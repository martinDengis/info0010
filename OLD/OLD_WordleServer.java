 ...
 * It manages entering connections by delegating to the OLD_ClientHandler class
 ...
 */
public class OLD_WordleServer {
    private static final int PORT = 2348;
    
    public static void main(String[] args) {
        int currentConnectionID = -1; // Keep track of number of connections
        List<OLD_ConnectionStatusMonitor> runningConnections = new ArrayList<>(); // Keep track of running threads
        ...
        try (ServerSocket ss = new ServerSocket(PORT)) {
            ...
            while (true) {
                ...
                try {
                    ...
                    OLD_ClientHandler client = new OLD_ClientHandler(clientSocket, secretWord, currentConnectionID);

                    Thread clientThread = new Thread(client);
                    runningConnections.add(new OLD_ConnectionStatusMonitor(clientThread, currentConnectionID));
                    clientThread.start();
    ...
    private static void isTerminated(List<OLD_ConnectionStatusMonitor> runningConnections) {
        for(int i = 0; i < runningConnections.size(); i++) {
            OLD_ConnectionStatusMonitor connectionStatus = runningConnections.get(i);
